...
const LoginForm = () => {
  ...
  const handleGoogleResponse = async (response) => {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success("Logged in with Google 🎉"); // <-- toast added
        localStorage.setItem("token", data.token);
        localStorage.setItem("user", JSON.stringify(data.user));
        navigate("/");
      } else toast.error(data.message || "Google login failed"); // <-- toast added
    } catch (err) {
      console.error(err);
      toast.error("Server error during Google login"); // <-- toast added
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username || !password) {
      toast.warn("Please enter both username and password"); // <-- toast added
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || "Invalid username or password"); // <-- toast added
      }
    } catch (err) {
      console.error(err);
      toast.error("Server error — please check backend connection."); // <-- toast added
    } finally {
      setLoading(false);
    }
  };
